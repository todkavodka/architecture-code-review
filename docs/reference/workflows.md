...
| `RESUME` и baseline изменился, advanced, diverged или unknown | вернуть `SOURCE_BASELINE_MISMATCH` и остановиться; показать review/revalidation и доступное согласованное действие |
| `EXTEND` и baseline изменился | вернуть `BASELINE_RECONCILIATION_REQUIRED` и остановиться |
...
| `PROJECTION_REPAIR` и источник изменился | заблокировать repair до reconciliation; исторический repair-режим не создаётся |
