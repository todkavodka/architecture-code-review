...
| `IN_PROGRESS` и изменившаяся базовая ревизия | `CHANGE_REVIEW` или `REVALIDATE`; при пригодном завершённом review — контекстный `RECONCILE_CHANGE` |
...
`RESUME` доступен только при совпадающем baseline. `RECONCILE_CHANGE` не
является startup intent; при несовпадении он доступен только контекстно и при
выполнении условий. Во всех случаях явный `NEW` остаётся доступен.

## Change Review и согласование baseline

`CHANGE_REVIEW` получает два явных источника: принятый `BASE` и выбранный
`CANDIDATE` (ветка, commit или pull request после разрешения в точную
ревизию/дерево). Его результаты ограничены кандидатным режимом и не меняют
принятые факты или проекции.

Сценарий продолжения имеет отдельные остановки:

| Ситуация | Действие |
|---|---|
| `RESUME` и baseline совпадает | восстановить состояние и продолжить первый незавершённый рубеж |
| `RESUME` и baseline изменился, advanced, diverged или unknown | вернуть `SOURCE_BASELINE_MISMATCH`, остановить `RESUME`; предложить `CHANGE_REVIEW`, `REVALIDATE` и при выполненных условиях контекстный `RECONCILE_CHANGE` |
| `EXTEND` и baseline изменился | вернуть `BASELINE_RECONCILIATION_REQUIRED` и заблокировать `EXTEND` |
| `REVALIDATE` | переоценить принятое состояние по затронутому срезу; complete Change Review — только routing evidence |
| `PROJECTION_REPAIR` и источник совпадает | исправлять только выбранную текущую проекцию |
| `PROJECTION_REPAIR` и текущий источник изменился | заблокировать текущий repair до reconciliation; исторический repair-режим не создаётся |

`RECONCILE_CHANGE` не является startup intent. После явного подтверждения оно
маршрутизирует минимальные candidate slices существующим владельцам. Baseline
продвигается только после полного учёта материальных изменений и прохождения
обязательных owner/technical/coverage gates. Это не approval выпуска или merge.

### Безопасное переиспользование и проекции

Результат можно переиспользовать для эквивалентного merge только с
`WHOLE_TREE_EQUAL` или `FROZEN_RELEVANT_SCOPE_EQUAL` proof. Candidate prediction
не устанавливает `CURRENT`, `STALE` или `BLOCKED`. После стабилизации принятого
изменения один раз выполняется `Projection Impact Analysis`; явная генерация
остаётся отдельным `RG-*` действием.
...
Для подробного API-среза разрешение зависимостей фиксирует точный Project или
Product member, baseline, направление, protocol kind, родительские IF и
выбранные разделы. Проекция перечисляет каждую принятую учтённую операцию и
показывает `PARTIAL`, `UNKNOWN` или `UNRESOLVED`, если часть кандидатов нельзя
разрешить точно. `Architecture Review` с `STANDARD_FULL` или `FORENSIC`
закрывает материальную поверхность, но сам по себе не запрашивает
перечисление всех endpoints. Формулировка о полном API разрешена только при
`OPERATION_INVENTORY_COMPLETE` для этого подробного среза и действительном
снимке зависимостей; `CURRENT` без такого inventory недостаточен.
...
**Условия входа:** у пакета есть незавершённая устойчивая граница и baseline
совпадает.
...
владеющего артефакта либо несовпадающий baseline. При несовпадении `RESUME`
останавливается с `SOURCE_BASELINE_MISMATCH` и предлагает `CHANGE_REVIEW`,
`REVALIDATE` и, если условия выполнены, контекстный `RECONCILE_CHANGE`.
...
Если `EXTEND` добавляет подробный `Provided Interfaces`, `Consumed Interfaces`
или `API Report`, а принятый STM содержит только surface depth, в
`resolved_work` добавляется адресное operation-depth enrichment: точный
Project/baseline (либо Product member/baseline), направление, protocol kind,
родительский IF-срез и evidence scope. Это не включает Architecture Review,
Test Engineering или Code Quality Review, не расширяет пользовательский
выбор и не пересобирает несвязанные проекции. Повторно используются принятые
surface facts; после enrichment отдельное принятие Technical Model Coverage
определяет, есть ли `OPERATION_INVENTORY_COMPLETE` или видимое ограничение.
...
Для подробных API-проекций добавление или удаление operation child, изменение
method/path или parent binding меняет membership/identity snapshot; изменение
schema, auth, boundary или evidence обновляет соответствующую семантическую
ревизию и зависимый срез по применимому контракту. Такие изменения делают
затронутую проекцию `STALE` и требуют явного `RG-*` после принятия обновлённой
семантики. Учёт влияния не сканирует исходники из приватного renderer path и
никогда не запускает regeneration автоматически.
