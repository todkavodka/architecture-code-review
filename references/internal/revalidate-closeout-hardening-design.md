...
- `references/session-orchestration.md` — baseline-role semantics remain owned by orchestration and are constrained by the hardening contract.
- `references/report-contract.md` — its existing authority map remains the source of truth for Architecture authority roles.
